...
const SAMPLE_PLACES = [
  { id: '1', name: '중앙대학교 후문 입구', address: '서울 어쩌고 저쩌고', isCurrent: true },
  { id: '2', name: '홍대역 2번 출구', address: '서울 저쩌고 어쩌고', isCurrent: false },
];
...
function getRepeatLabel(repeat: string[]): string {
  ...
  const isWeekday = weekdays.every((d) => repeat.includes(d)) && repeat.length === weekdays.length;
  const isWeekend = weekend.every((d) => repeat.includes(d)) && repeat.length === weekend.length;
  const isEvery = all.every((d) => repeat.includes(d));
  if (isEvery) return '매일';
  if (isWeekday) return '주중';
                ...
                return (
                  <View key={place.id}>
                    <TouchableOpacity
                      ...
                      onPress={() => setTempPlace(place.name)}
                    >
