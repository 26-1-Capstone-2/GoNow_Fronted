...
export function enterReadyGeofenceMode(
  ...
  return withRegionLock(async () => {
    ...
    dlog('READY', `key:${key} 등록/재센터링 완료 — 앵커(${anchorLat.toFixed(6)}, ${anchorLng.toFixed(6)}) 목적지(${destLat.toFixed(6)}, ${destLng.toFixed(6)})`);
  });
}
  ...
  await withKeyLock(key, async () => {
    ...
    try {
      ...
      if (status === 'NEARDEST') {
        ...
        // 앵커 500m 이탈에 따른 재계산 결과 여전히 READY — 방금 좌표로 재센터링.
        // enterReadyGeofenceMode()가 항상 덮어쓰는 구조라(다른 상태로의 핸드오프와 달리 같은
        // READY 지오펜스 태스크 안에서의 갱신이므로) exitReadyGeofenceMode()를 먼저 호출할
        // 필요가 없다 — 그러면 네이티브 재등록이 불필요하게 2번(해제+등록) 일어나고, 위 "지오펜스
        // 콜백 신뢰성" 섹션의 짧은 시간 재등록 경쟁 위험도 괜히 키운다. 한 번만 호출.
        dlog('READY', `key:${key} 여전히 READY — 재센터링`);
        await enterReadyGeofenceMode(key, coords.latitude, coords.longitude, nav?.destLat, nav?.destLng);
